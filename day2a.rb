...
result = input.split("\n").map do |line|
  ...
  [char, min.to_i, max.to_i, password.strip]
end

result.compact!

result = result.select do |char, min, max, password|
  number_of_chars =
    password.chars.select{ |c| c == char }.size

  number_of_chars <= max && number_of_chars >= min
end
