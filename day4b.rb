...
passports = input.map do |string|
  string.split(/\s/).map do |entry|
    entry.split(':')
  end.to_h
end
...
def valid_height?(hgt)
  case hgt[-2..-1]
  when 'cm'
    hgt[...-2].to_i >= 150 && hgt[...-2].to_i <= 193
  when 'in'
    hgt[...-2].to_i >= 59 && hgt[...-2].to_i <= 76
  else
    false
  end
end
...
def valid_passport?(passport)
  ...
  return false unless passport.keys.to_set.superset?(required_fields.to_set)
  ...
  return false unless passport['hcl'].match?(/^#[a-f0-9]{6}$/)
